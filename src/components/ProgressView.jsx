...
export default function ProgressView(props){
  const { sessions, C, Chip, EXAM_COLOR } = props;
  if(!sessions.length) return <div className="session-note">No sessions recorded yet. Complete your first practice session to see your progress.</div>;
        ...
        {last10.map((s,i)=>{
          ...
          return (
            <div key={i} className="score-col">
              ...
              <div style={{fontSize:10,color:C.muted,fontFamily:'var(--font-ui)'}}>{new Date(s.date).toLocaleDateString('en-GB',{day:'2-digit',month:'short'})}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="session-log">
        ...
        {[...sessions].reverse().map((s,i)=>(
          <div key={i} className="session-item">
            <div className="date">{new Date(s.date).toLocaleString('en-GB',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}</div>
            <Chip label={s.exam} color={EXAM_COLOR[s.exam]||C.accent} small/>
            <div style={{fontSize:12,color:Math.round(s.score/s.total*100)>=70?C.green:C.amber}}>{s.score}/{s.total} · {Math.round(s.score/s.total*100)}%</div>
          </div>
        ))}
      </div>
    </div>
