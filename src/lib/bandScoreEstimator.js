...
// Official IELTS Listening band table (updated 2025-2026 specifications)
...
export function estimateOverallBand(sessions) {
  ...
  // Official IELTS rounding: .25 → next half band, .75 → next whole band
  ...
  const overallBand = bandEstimates.length
    ? ieltsRound(bandEstimates.reduce(function (s, b) { return s + b; }, 0) / bandEstimates.length)
    : null;
